...
class CRSession(requests.Session):
    ...
    def _make_cookies(self):
        ...
        payload = {
            "token": self.cr_token_response.access_token
        }
        headers = {
        ...
        if response.status_code >= 400:
            raise Exception(f'could not get cookies status code: {response.status_code}')
    ...
    def cr_token_response(self) -> CR_TokenResponse:
        if (not self._cr_token_response) or (
                time.time() - self._cr_token_response.creation_time + 30 >= self._cr_token_response.expires_in):
            self._csrf_token = None
